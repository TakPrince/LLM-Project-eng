...
class AutonomousPlanningAgent(Agent):
    ...
    # 🔁 CHANGED MODEL (cheap + powerful + tool calling)
    ...
    def __init__(self, collection):
        ...
        # 🔁 CHANGED CLIENT (OpenRouter OpenAI-compatible)
        self.openai = OpenAI(
    ...
    def handle_tool_call(self, message):
        ...
        Modified to handle Llama-style argument lists.
        """
        mapping = {
        ...
        for tool_call in message.tool_calls:
            ...
            # 2. 🛠️ FIX: If Llama sends arguments as a list [{}], extract the dict
            if isinstance(arguments, list) and len(arguments) > 0:
                arguments = arguments[0]
            
            # 3. Final check to ensure we have a dictionary before unpacking
            if not isinstance(arguments, dict):
                self.log(f"Warning: Arguments for {tool_name} were not a dict. Found: {type(arguments)}")
                arguments = {}

            tool = mapping.get(tool_name)
            
            # 4. Call the tool and catch results
            try:
                result = tool(**arguments) if tool else f"Error: {tool_name} not found"
            except TypeError as e:
                self.log(f"Error calling {tool_name}: {e}")
                result = f"Error executing {tool_name}"
